...
function doAlg(algorithm){
    if(disappearOnNextMove){
        if(!/[xyzU]/.test(algorithm)){
            greyLSE();
            disappearOnNextMove = false;
        }
    }
...
function greyLSE(){
    if(cube.hasGreyStickers()){
        ...
        cube.greyStickers([1,3,4,5,7]);
        cube.greyStickers([19, 22, 25]);
        cube.greyStickers([28, 31, 34]);
...
listener.simple_combo("1", function() {	greyLSE();
drawCube(cube.cubestate);});
...
function RubiksCube() {
    ...
    this.greyStickers = function(arr){
        var i;
        for(i = 0; i < arr.length; i++){
            this.cubestate[arr[i]] = -1*this.cubestate[arr[i]];
        }
    }
    
    this.hasGreyStickers = function(){
        var i;
        for(i=0; i<this.cubestate.length;i++){
            if(this.cubestate[i]<0){
                return true;
            }
        }
        return false;
    }
    this.unGrey = function(){
        var i;
        for(i = 0; i < this.cubestate.length; i++){
            if(this.cubestate[i]<0){
                this.cubestate[i] = -1*this.cubestate[i];
            }
        }
    }
